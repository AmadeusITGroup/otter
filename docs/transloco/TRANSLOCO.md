...
The transloco module is built on top of [JSVerse Transloco](https://github.com/jsverse/transloco).
...
- Resource keys can be translated from templates (`*.html`) via a [pipe](https://github.com/jsverse/transloco/blob/main/libs/transloco/src/lib/transloco.pipe.ts) or [directive](https://github.com/jsverse/transloco/blob/main/libs/transloco/src/lib/transloco.directive.ts) as well as from typescript (`*.ts`) via a [service](https://github.com/jsverse/transloco/blob/main/libs/transloco/src/lib/transloco.service.ts).
...
We provide localization providers in **@o3r/transloco** via the [provideLocalization](https://github.com/AmadeusITGroup/otter/blob/main/packages/@o3r/transloco/src/tools/localization-module.ts) function which comes with a translations loader.

## Configuration of Localization Providers

In your `AppModule` (or standalone app configuration), you need to add the localization providers using `provideLocalization()` alongside Transloco providers (from **@jsverse/transloco**). The `provideLocalization()` function accepts a custom configuration object or factory to specify the language of the application. This configuration is of type `LocalizationConfiguration` and describes:
...
All the possible configuration options can be found in the `LocalizationConfiguration` [interface](https://github.com/AmadeusITGroup/otter/blob/main/packages/%40o3r/transloco/src/core/localization-configuration.ts).

You can also call `provideLocalization()` without a configuration parameter. In this case, you will be provided with some default `LocalizationConfiguration`.
...
import { provideLocalization, translocoLoaderProvider } from '@o3r/transloco';
...
@NgModule({
  ...
  providers: [
    provideTransloco({
      ...
      loader: translocoLoaderProvider
    }),
...
import localeFR from '@angular/common/locales/fr';
import { provideTransloco } from '@jsverse/transloco';
import { LOCALIZATION_CONFIGURATION_TOKEN, provideLocalization, translocoLoaderProvider } from '@o3r/transloco';
// ...
...
> [!NOTE]
> The `ngAdd` of the **@o3r/transloco** package will set the `useLocalization` parameter to true for all schematic packages beginning with `'@o3r/core:component*'`
...
import { Localization, O3rLocalizationTranslatePipe, Translatable } from '@o3r/transloco';
import { SimpleHeaderPresTranslation, translations } from './simple-header-pres-translation';

@Component({
  selector: 'o3r-simple-header-pres',
  imports: [O3rLocalizationTranslatePipe],
...
> [!NOTE]
> The code to the `o3rTranslate` pipe can be found in the [tools of the @o3r/transloco module](https://github.com/AmadeusITGroup/otter/blob/main/packages/%40o3r/transloco/src/tools/localization-translate-pipe.ts).
...
Use Angular's built-in [DatePipe](https://angular.io/api/common/DatePipe), [DecimalPipe](https://angular.io/api/common/DecimalPipe) and [CurrencyPipe](https://angular.io/api/common/CurrencyPipe),
or use the Otter localized pipes: `LocalizedDatePipe`, `LocalizedDecimalPipe`, and `LocalizedCurrencyPipe`. These pipes automatically use the current locale from the `LocalizationService`.

To use Angular's pipes with the current locale, pass `this.localizationService.getCurrentLanguage()` as the last parameter. You will need to inject the `LocalizationService` in your component.

```typescript
import { Component, inject, Input } from '@angular/core';
import { Localization, LocalizationService, O3rLocalizationTranslatePipe, Translatable } from '@o3r/transloco';
import { LocalizedDatePipe, LocalizedDecimalPipe, LocalizedCurrencyPipe } from '@o3r/transloco';
import { SimpleHeaderPresTranslation, translations } from './simple-header-pres-translation';

@Component({
  selector: 'o3r-simple-header-pres',
  imports: [O3rLocalizationTranslatePipe, LocalizedDatePipe, LocalizedDecimalPipe, LocalizedCurrencyPipe],
  styleUrls: ['./simple-header-pres.scss'],
  templateUrl: './simple-header-pres.html'
})
export class SimpleHeaderPres implements Translatable<SimpleHeaderPresTranslation>, ... {
  ...
  public readonly currentLanguage = toSignal(
    this.localizationService.getTranslateService().langChanges$.pipe(map(lang => lang)),
...
```typescript
import { Translatable } from '@o3r/transloco';
...
import { LocalizationService } from '@o3r/transloco';

// Inject LocalizationService which will take care of configuring TranslocoService using LocalizationConfiguration and call configure() method
constructor() {
...
import { inject } from '@angular/core';
import { TextDirectionService } from '@o3r/transloco';
...
### ICU Translation Support with Message Format

To handle ICU translations (pluralization, gender selection), use the `provideTranslocoMessageformat()` provider from `@jsverse/transloco-messageformat`.
...
import localeFR from '@angular/common/locales/fr';
import { provideTransloco } from '@jsverse/transloco';
import { provideLocalization, translocoLoaderProvider, TRANSLOCO_MESSAGE_FORMAT_CONFIG } from '@o3r/transloco';
...
@NgModule({
  ...
  providers: [
    ...
    {provide: TRANSLOCO_MESSAGE_FORMAT_CONFIG, useValue: {enableCache: false}}
  ]
})
class AppModule {}
```
...
> [!NOTE]
> The token `TRANSLOCO_MESSAGE_FORMAT_CONFIG` implements the `MessageformatConfig` interface from `@jsverse/transloco-messageformat`.

### How to localize plural expressions

For pluralization, we are using the `TranslateMessageFormatLazyTranspiler`, which extends `MessageFormatTranspiler` from the [@jsverse/transloco-messageformat](https://www.npmjs.com/package/@jsverse/transloco-messageformat) package. This transpiler uses `messageformat.js` to compile translations using ICU syntax for handling pluralization and gender.
...
#### Integration with Transloco

You need to configure the Transloco providers with `provideTranslocoMessageformat()`. See the [ICU Translation Support with Message Format](#icu-translation-support-with-message-format) section above for details.
...
import { provideTransloco } from '@jsverse/transloco';
import { provideTranslocoMessageformat } from '@jsverse/transloco-messageformat';
import { provideLocalization, TRANSLOCO_MESSAGE_FORMAT_CONFIG, translocoLoaderProvider } from '@o3r/transloco';
...
@NgModule({
  ...
  providers: [
    ...
    // Optional transpiler configuration:
    {provide: TRANSLOCO_MESSAGE_FORMAT_CONFIG, useValue: {locales: ['ar', 'fr']}}
  ]
})
class AppModule {}
```
...
> [!NOTE]
> This mechanism only applies to the pipe and directive exported by Otter's ``LocalizationModule``. The original ones from ``@jsverse/transloco`` do not support it.
...
@NgModule({
  providers: [
    {provide: LOCALIZATION_CONFIGURATION_TOKEN, useFactory: localizationConfigurationFactory},
    provideLocalization()
  ]
})
export class AppModule {}
...
To enable the communication with the [Otter Devtools](https://chrome.google.com/webstore/detail/otter-devtools/cgbnjkegcpddliemjooeiomcdnacbdol), the following steps are required:

1. Add the localization devtools provider to your application:
...
2. Inject the `LocalizationDevtoolsMessageService` in the `main.ts` file of the application (we recommend doing this to avoid polluting the `app.ts` file):
...
import { LocalizationDevtoolsConsoleService, LocalizationDevtoolsMessageService } from '@o3r/transloco';
import { AppModule } from './app/app-module';

platformBrowserDynamic().bootstrapModule(AppModule).then((m) => runInInjectionContext(m.injector, () => {
  inject(LocalizationDevtoolsConsoleService);
  inject(LocalizationDevtoolsMessageService);
}))
```

3. The debug message service can be activated on bootstrap:

You can do this in the `main.ts` file:

```typescript
// main.ts
platformBrowserDynamic().bootstrapModule(AppModule).then((m) => runInInjectionContext(m.injector, () => inject(LocalizationDevtoolsConsoleService).activate()));
```

Or pass options to the provider in the `AppModule`:
...
import { provideLocalizationDevtools, OTTER_LOCALIZATION_DEVTOOLS_OPTIONS } from '@o3r/transloco';
...
[Link to the `@o3r/transloco` package](https://github.com/AmadeusITGroup/otter/blob/main/packages/%40o3r/transloco/README.md).
