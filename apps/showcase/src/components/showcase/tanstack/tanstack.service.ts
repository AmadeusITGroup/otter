...
export class TanstackService {
  ...
  public mutationUploadFile = injectMutation((client: any) => ({
    mutationFn: (petFile: {petId: number; body: any}) => this.petStoreApi.uploadFile(petFile),
    onSuccess: () => {
      // Invalidate and refetch by using the client directly
      client.invalidateQueries({ queryKey: ['findPetsByStatus'] });
    }
  }));


  public mutationAdd = injectMutation((client: any) => ({
  ...
  public mutationDelete = injectMutation((client: any) => ({
