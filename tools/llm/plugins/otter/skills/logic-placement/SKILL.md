...
description: Decide where new logic belongs — component shape, logic layer, and store variant — when designing a component, service, or store in an Angular/Otter workspace.
  Use `logic-review` to assess existing code.
---

# Logic placement — design mode

Decide where a piece of logic belongs **before** it is written.
This skill holds the order of the decisions, the rules for halting, and the output format.
The criteria for each decision live in a sub-skill: read it before answering, do not answer from memory.
...
The store implementation decision runs **only** if the logic layer decision returned "store".
Never start there: most state that feels store-shaped is service-shaped, and most components that feel split-shaped are not.
...
```
TRIGGER  a container/presenter split is suggested, but none of the variation
...
RECOMMENDATION  <name>-cont + <name>-pres
  ...
  because       <which variation criterion applies, and what varies>
...
Run the NEXT STEP through `otter-schematics`.
If the schematic fails or does not exist, do not hand-write the files from memory — that skill covers reading the schematic's own templates and reproducing them.
