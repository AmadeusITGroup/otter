---
name: logic-review
description: This skill should be used when reviewing existing components, services, or stores for whether logic sits in the right place — "review this component", "is this logic in the right place", "should this be in the container", "is this store type right", "check this presenter" — or when reviewing a pull request that touches components, services, or stores. Detects misplaced logic, presenter leaks, componentType mismatches, and wrong store types, and reports findings. Use `logic-placement` instead when designing something that does not exist yet.
---
...
- **Judge a split by what varies, not by whether it exists.** A missing split is a finding only when one of the three variation criteria is met. An existing split whose container only forwards inputs is a finding regardless.
...
- [Container / Presenter](https://github.com/AmadeusITGroup/otter/blob/main/docs/components/CONTAINER_PRESENTER.md) — the source for the split, the naming conventions, and the folder layout a finding can cite
