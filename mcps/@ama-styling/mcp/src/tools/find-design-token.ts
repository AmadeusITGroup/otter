...
export function scoreTypeCompatibility(
  ...
  if (isColorProperty && tokenType && tokenType !== 'color') {
    return { score: -1, reasons: [`token type "${tokenType}" does not match color CSS property`] };
  }
  if (!isColorProperty && tokenType === 'color') {
    return { score: -1, reasons: [`color token does not match non-color CSS property "${cssProperty}"`] };
  }
  return { score: 0, reasons: ['no type compatibility signal'] };
}
...
export function registerFindDesignTokenTool(server: McpServer, options: FindDesignTokenOptions): void {
  ...
  server.registerTool(
    ...
    async ({ cssProperty, context, maxResults }) => {
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                ...
                reasons: [
                  `No tokens scored positively for the given CSS property "${cssProperty}" and context "${context}" combination`,
                  `${tokens.length} token(s) were evaluated but none matched the provided criteria`
                ],
                suggestions: [
      ...
      return {
        content: [
          { type: 'text' as const, text: JSON.stringify(scored) }
        ]
      };
    }
  );
