...
describe('registerListDesignTokensTool', () => {
  ...
  it('should return all tokens when no filters provided', async () => {
    ...
    const { tokens } = JSON.parse(result.content[0].text) as { tokens: TokenResult[] };
  ...
  it('should return all tokens with warnings for invalid regex namePattern', async () => {
    const result = await toolCallback({ namePattern: '[invalid(' });
    const parsed = JSON.parse(result.content[0].text) as { warnings: string[]; tokens: TokenResult[] };

    expect(parsed.warnings).toHaveLength(1);
    expect(parsed.warnings[0]).toContain('Invalid regex pattern');
    expect(parsed.warnings[0]).toContain('[invalid(');
    expect(parsed.tokens).toHaveLength(4);
  });

  it('should return all tokens with warnings for unsafe regex patterns (ReDoS protection)', async () => {
    const result = await toolCallback({ namePattern: '(a+)+' });
    const parsed = JSON.parse(result.content[0].text) as { warnings: string[]; tokens: TokenResult[] };

    expect(parsed.warnings).toHaveLength(1);
    expect(parsed.warnings[0]).toContain('catastrophic backtracking');
    expect(parsed.tokens).toHaveLength(4);
  });

  it('should return all tokens with warnings for regex patterns exceeding maximum length', async () => {
    const result = await toolCallback({ namePattern: 'a'.repeat(201) });
    const parsed = JSON.parse(result.content[0].text) as { warnings: string[]; tokens: TokenResult[] };

    expect(parsed.warnings).toHaveLength(1);
    expect(parsed.warnings[0]).toContain('too long');
    expect(parsed.tokens).toHaveLength(4);
  });
