...
describe('scoreTypeCompatibility', () => {
  ...
  it('should penalize non-color tokens for color CSS properties', () => {
    expect(scoreTypeCompatibility('background-color', 'string')).toEqual({ score: -1, reasons: [] });
  });

  it('should penalize color tokens for non-color CSS properties', () => {
    expect(scoreTypeCompatibility('font-size', 'color')).toEqual({ score: -1, reasons: [] });
  });

  it('should return neutral for non-color property and non-color token', () => {
    expect(scoreTypeCompatibility('font-size', 'string')).toEqual({ score: 0, reasons: [] });
  });

  it('should return neutral when token type is undefined', () => {
    expect(scoreTypeCompatibility('font-size', undefined)).toEqual({ score: 0, reasons: [] });
  });
});
