...
import { getCliModules, getFormattedDescription, getInstalledPath, installDependency, isInstalled } from '../helpers/module.helper';

export const amaYargs = async (argv?: Record<string, any>) => {

  /** Module registered to Amaterasu CLI */
  const modules = await getCliModules(dependencies, { localOnly: !!argv?.['local-only'] });
  ...
  const baseYargs = yargsAmaCli
    ...
    .group(['yes', 'verbose', 'config', 'local-only'], 'Common:')
  ...
  return modules
    .reduce((acc, mod) => {
      ...
      return acc
        .command(mod.moduleName, getFormattedDescription(mod), async (y) => {
          if (!isInstalled(mod)) {
            ...
            mod.resolutionPath = getInstalledPath(mod);
            if (!isInstalled(mod)) {
              throw new Error(`Something goes wrong with the installation of ${mod.name}`);
            }
          }

          (require(mod.resolutionPath) as AmaCliModule).init((version ? y.version(version) : y).usage(generateUsageMessage(mod.moduleName, '<commands...>')), baseContext);
        }
        );
    }, baseYargs);
};
