...
export function getExternalDependenciesInfo<T extends string, U extends string>(
  ...
  shouldEnforceTildeRange = (name: string) => !/^@?angular/.test(name) && !name.startsWith('@ngrx'),
  ...
  const peerDependenciesInfo = dependenciesToInstall.reduce((acc, name) => {
    ...
    let range = rootVersion || getVersionToInstallFromPackageJson(name, o3rPackageJsonPath, shouldEnforceTildeRange(name));
