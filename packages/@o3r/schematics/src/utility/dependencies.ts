 ...
 * @param logger
 */
export function getVersionToInstallFromPackageJson(packageName: string, packageJsonPath: string, isTildeEnforced?: boolean, logger?: logging.LoggerApi): string {
...
export const isAngularDependency = (name: string) => name.match(/^@?angular/) || name.match(/^@ngrx/);
...
export function isDependencyAlreadyInstalled(depName: string, packageJson: PackageJson, {
  ...
  return packageJsonDepVersions.some((version) => !!version && (satisfies(version, range) || subset(version, range)));
}
...
export function getExternalDependenciesInfo<T extends string, U extends string>(
  ...
  const peerDependenciesInfo = dependenciesToInstall.reduce((acc, name) => {
    const range = getVersionToInstallFromPackageJson(name, o3rPackageJsonPath, shouldEnforceTildeRange(name), logger);
    const inManifest = {
      range,
