...
import { getProjectFromTree, getTemplateFolder } from '../../utility/loaders';
...
export function createAzurePipeline(options: { projectName: string | null; enableStorybook: boolean }, rootPath: string): Rule {
  ...
  const manualProcessNotification = (tree: Tree, context: SchematicContext) => {
    ...
    context.logger.info('To be able to deploy your application to Deploy Service V2, you need to manually process the following steps:');
    ...
    context.logger.info('  - follow the instructions to create your application blueprint');
    context.logger.info('    https://dev.azure.com/AmadeusDigitalAirline/Otter/_wiki/wikis/Otter%20library%20documentation/1810/AZURE_DEVOPS');
