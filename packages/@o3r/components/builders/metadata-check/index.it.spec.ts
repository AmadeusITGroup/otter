...
const initTest = async (
  builderOptions: ConfigMigrationMetadataCheckBuilderSchema,
  newMetadata: ComponentConfigOutput[],
  migrationData: MigrationFile<MigrationConfigData>,
  packageNameSuffix: string
) => {
  const { workspacePath, appName, applicationPath, o3rVersion } = o3rEnvironment.testEnvironment;
  ...
  let version;
  try {
    version = getLatestPackageVersion(packageName, execAppOptionsWorkspace);
  } catch {
    version = baseVersion;
  }

  const bumpedVersion = inc(version, 'patch');
...
describe('check metadata migration', () => {
  ...
  test('should throw because no migration data', async () => {
    ...
    try {
      ...
      previousConfigurationMetadata.slice(1).forEach(({ library, name, properties }) => {
        expect(e.message).toContain(`Property ${library}#${name} ${properties[0]!.name} has been modified but is not documented in the migration document`);
      });
    }
  });
  ...
  test('should throw because migration data invalid', async () => {
    ...
    try {
      ...
      expect(e.message).not.toBe('should have thrown before');
      previousConfigurationMetadata.slice(1).forEach(({ library, name, properties }) => {
        expect(e.message).toContain(`Property ${library}#${name} ${properties[0]!.name} has been modified but the new property is not present in the new metadata`);
      });
    }
  });
  ...
  test('should throw because breaking changes are not allowed', async () => {
    ...
    try {
      ...
      expect(e.message).not.toBe('should have thrown before');
      previousConfigurationMetadata.slice(1).forEach(({ library, name, properties }) => {
        expect(e.message).toContain(`Property ${library}#${name} ${properties[0]!.name} is not present in the new metadata and breaking changes are not allowed`);
      });
    }
  });

  test('should throw because deletion are not allowed', async () => {
    await initTest(
      {
        granularity: 'minor',
        metadataPath: './component.config.metadata.json',
    ...
    try {
      ...
      expect(e.message).not.toBe('should have thrown before');
      previousConfigurationMetadata.slice(1).forEach(({ library, name, properties }) => {
        expect(e.message).toContain(`Property ${library}#${name} ${properties[0]!.name} is not present in the new metadata`);
      });
    }
  });
});
