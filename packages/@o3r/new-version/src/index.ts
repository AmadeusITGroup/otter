...
export class NewVersion {
  ...
  public computeNewVersion(tags: string[], versionMask: string) {
    ...
    if (this.isDefaultBranch) {
      const releaseTags = new Set([...this.defaultBranchPrereleaseName ? [this.defaultBranchPrereleaseName] : [], 'prerelease', 'rc']);
      parsedSortedTags = parsedSortedTags.filter((parsedTag) =>
        parsedTag.prerelease.length === 0 || releaseTags.has(`${parsedTag.prerelease[0]}`)
      );
    } else {
