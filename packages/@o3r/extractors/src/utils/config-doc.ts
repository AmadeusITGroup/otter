...
export function getCategoriesFromDocText(docComment: string): CategoryDescription[] | undefined {
  ...
  if (categoriesString) {
    for (const cat of categoriesString) {
      const firstSpaceIndex = cat.indexOf(' ');
      if (firstSpaceIndex < 1) {
        const catName = cat;
        const catLabel = cat.charAt(0).toUpperCase() + cat.slice(1);
        categoriesWithDescription.push({name: catName, label: catLabel});
      } else {
        const catName = cat.slice(0, firstSpaceIndex);
        const catLabel = cat.slice(firstSpaceIndex + 1);
        categoriesWithDescription.push({name: catName, label: catLabel});
      }
    }
  }
