...
function checkMetadataFile<MetadataItem, MigrationMetadataItem, MetadataFile>(
  ...
  for (const lastValue of lastMetadataArray) {
    ...
    if (!isInNewMetadata) {
      ...
      errors.push(new Error(`Property ${comparator.getIdentifier(lastValue)} is not present in the new metadata`));
    }
  }
  return errors;
}
...
Detection of package manager runner will fallback on the one used to execute the actual command.`);
  ...
  const projectRoot = context.target && angularJson && angularJson.projects[context.target.project].root || context.currentDirectory;
  ...
  const migrationDataFiles = globbySync(options.migrationDataPath);
  // TODO : will not work when running from apps/test-app -> should join from the root folder if relative?
  // Discrepancy between metadataPath and migrationDataPath
  // metadataPath is posix.join(projectRoot, options.metadataPath)
  const {path: migrationFilePath, version: currentVersion} = getLatestMigrationMetadataFile(migrationDataFiles) || {};

  if (!migrationFilePath || !currentVersion) {
    throw new Error(`No migration data could be found matching ${typeof options.migrationDataPath === 'string' ? options.migrationDataPath : options.migrationDataPath.join(',')}`);
  }

  context.logger.info(`Latest version present in migration data folder: ${currentVersion}`);
  const previousVersion = getVersionRangeFromLatestVersion(currentVersion, options.granularity);
