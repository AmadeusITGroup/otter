...
export default createBuilder<AggregateMigrationScriptsSchema>(createBuilderWithMetricsIfInstalled(async (options, context): Promise<BuilderOutput> => {
  ...
  try {
    context.reportProgress(1, STEP_NUMBER, 'Find all migration files');
    ...
    context.reportProgress(2, STEP_NUMBER, 'Fetch migration files from libraries and aggregate the changes');
    ...
    context.reportProgress(3, STEP_NUMBER, 'Write the output');
