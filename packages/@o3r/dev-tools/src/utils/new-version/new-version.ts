...
export class NewVersion {
  ...
  constructor(private options: NewVersionOptions) {
  ...
  public async retrieveGitTags() {
    ...
    this.options.logger.verbose(`Executing command: ${gitTagCommand}`);
  ...
  public computeNewVersion(tags: string[], versionMask: string) {
    ...
    this.options.logger.verbose('Parsed and sorted tags:');
    this.options.logger.verbose(parsedSortedTags);
    this.options.logger.verbose(`Version mask: ${versionMask}`);
    ...
    this.options.logger.verbose('Tags after filtering:');
    this.options.logger.verbose(parsedSortedTags);
    ...
    if (this.isDefaultBranch) {
      if (!latest) {
        ...
        this.options.logger.verbose(`Bumping patch ${this.defaultBranchPrereleaseName || ''}`);
    ...
    this.options.logger.verbose(`Latest version from tags: ${latest?.version}`);
    this.options.logger.verbose(`Base version computed: ${baseVersion}`);
