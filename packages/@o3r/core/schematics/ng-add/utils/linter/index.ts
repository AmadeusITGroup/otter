...
export const shouldOtterLinterBeInstalled = async (context: SchematicContext): Promise<boolean> => {
  ...
  try {
    const eslintPackageJsonPath = require.resolve(`${linterPackageName}/package.json`);
...
Otherwise, you can add them later via this command: ng add @o3r/eslint-config${lt(eslintInstalledVersion, '9') ? '-otter' : ''}`, true);
    }
  } catch {
