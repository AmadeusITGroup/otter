...
describe('createEntityAsyncRequestAdapter tests', () => {
  ...
  const emptyState: TestState = testAdapter.getInitialState<TestState>({ requestIds: [] });

  const emptyStateWithOtherId: TestStateWithCustomId = testAdapterWithCustomId.getInitialState<TestStateWithCustomId>({ requestIds: [] });
