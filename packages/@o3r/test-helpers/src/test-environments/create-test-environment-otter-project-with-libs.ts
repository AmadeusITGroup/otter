...
export interface CreateTestEnvironmentOtterProjectWithLibOptions extends CreateWithLockOptions, PackageManagerConfig {
  /**
   * Name of the lib to generate
   */
  projectName: string;
...
/**
 * Generate a base angular library with minimal necessary dependencies
 * Uses a locker mechanism so this function can be called in parallel
 * The lock will automatically expire after 10 minutes if the creation of the library failed for whatever reason
 * @param inputOptions
 */
export async function createTestEnvironmentOtterProjectWithLib(inputOptions: Partial<CreateTestEnvironmentOtterProjectWithLibOptions>) {
  const options: CreateTestEnvironmentOtterProjectWithLibOptions = {
    projectName: 'test-lib',
    appDirectory: 'test-lib',
    o3rVersion,
    cwd: process.cwd(),
  ...
  await createWithLock(() => {
    ...
    packageManagerExec({ script: 'ng', args: ['g', 'library', 'dont-modify-me'] }, execAppOptions);
    packageManagerExec({ script: 'ng', args: ['g', 'library', options.projectName] }, execAppOptions);
