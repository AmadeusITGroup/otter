 ...
 * @jest-environment-o3r-app-folder test-app-rule-fact-metadata-check
 */
...
const metadataFileName = 'rules.facts.metadata.json';

const defaultMigrationData: MigrationFile<MigrationRuleFactMetadata> = {
  version,
  changes: [
    { // Rename key name
      contentType: 'RULE_FACT',
      before: {
        name: 'old-fact1'
      },
      after: {
...
const createMetadataFact = (name: string): MetadataFact => ({
  name,
  type: 'boolean'
});

const previousRuleFactMetadata: MetadataFact[] = [
  createMetadataFact('unchanged-fact'),
  createMetadataFact('old-fact1')
];

const newRuleFactMetadata: MetadataFact[] = [
  createMetadataFact('unchanged-fact'),
  createMetadataFact('new-fact1')
];
...
const initTest = async (
  newMetadata: MetadataFact[],
  migrationData: MigrationFile<MigrationRuleFactMetadata>,
  ...
  packageManagerExec({ script: 'ng', args: ['add', `@o3r/rules-engine@${o3rVersion}`, '--skip-confirmation', '--project-name', appName] }, execAppOptionsWorkspace);
  ...
  await writeFileAsJSON(metadataPath, previousRuleFactMetadata);
...
describe('check metadata migration', () => {
  ...
  test('should throw because no migration data', async () => {
    ...
    try {
      ...
      previousRuleFactMetadata.slice(1).forEach(({ name: id }) => {
