...
const updateStyleDictionaryConfig: Rule = (tree, context) => {
  ...
  configFiles.forEach((file) => {
    ...
    if (text.includes('@o3r/style-dictionary')) {
      ...
      tree.overwrite(file, text.replace(/o3r/g, 'ama-styling'));
    }
  });
};
...
export const updateAll = createOtterSchematic(() => chain([
  ...
  (_, context) => {
    context.logger.warn('The package "@o3r/style-dictionary" is deprecated, please install "@ama-sdk/style-dictionary" with the following command:');
    context.logger.warn('ng add @ama-sdk/style-dictionary');
  }
]));
