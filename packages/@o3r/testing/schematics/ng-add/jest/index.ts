...
/**
 * Add jest setup files to eslint tsconfig to ensure they are correctly linted
 * @param workingDirectory
 */
function setJestFilesInEslintConfig(workingDirectory: string) {
  return (tree: Tree) => {
    const eslintPath = path.posix.join(workingDirectory, 'tsconfig.eslint.json');
    const eslintConfig = tree.readJson(eslintPath) as { include?: string[] };
    eslintConfig.include = Array.from(new Set([...(eslintConfig.include || []), 'testing/setup-jest.ts']));
    tree.overwrite(eslintPath, JSON.stringify(eslintConfig, null, 2));
  };
}
 ...
 * @param options
 */
export function setupJest(options: NgAddSchematicsSchema & { eslintSetupRequired: boolean }): Rule {
  return (tree: Tree, context: SchematicContext) => {
    ...
    const rules = [
      ...
      setupJestInProject(),
      ...options.eslintSetupRequired ? [setJestFilesInEslintConfig(workingDirectory)] : []
    ];
