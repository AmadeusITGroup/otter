import type { AnalyticsThirdPartyService } from '../analytics-third-party.interfaces';
 ...
 * @param options.uuid UUID of the Google Analytics Account identifying the application
 */
export function createGoogleAnalyticsService({ uuid }: GoogleAnalyticsServiceOptions): AnalyticsThirdPartyService {
  ...
  return {
    onRegistration: () => {
      ...
      gtag = function (...args: any[]) { dataLayer.push(args); };
      gtag('js', new Date());
      gtag('config', uuid);
    },
