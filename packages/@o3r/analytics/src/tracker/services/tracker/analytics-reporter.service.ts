...
import { ANALYTICS_REPORTER_CONFIGURATION, type AnalyticsReporterConfiguration } from './analytics-reporter.configuration';
import type { AnalyticsHookOptions, AnalyticsThirdPartyService } from '../../third-party/analytics-third-party.interfaces';
import type { AnalyticsAvailableEvents, ReportedEvent } from '../../events/base.interface';
...
export class AnalyticsEventReporter {
  ...
  constructor(@Inject(ANALYTICS_REPORTER_CONFIGURATION) readonly config: AnalyticsReporterConfiguration) {
    ...
    this.events$ = this.stack.asObservable();
