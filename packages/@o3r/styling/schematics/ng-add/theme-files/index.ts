  ...
  return async (tree: Tree, context: SchematicContext) => {
    ...
    if (workspaceProject.architect?.build?.options?.assets) {
      workspaceProject.architect.build.options.assets
        = workspaceProject.architect.build.options.assets.filter((a: { glob: string; input: string; output: string }) => !a.input || !a.input.includes('node_modules/@otter/styling/assets'));
    }
