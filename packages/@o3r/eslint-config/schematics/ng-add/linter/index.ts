...
export function updateLinterConfigs(options: { projectName?: string | null | undefined }, rootPath: string): Rule {
  ...
  /**
   * Update or create the eslint.config.js file
   * @param tree
   * @param context
   */
  const updateTslintExtend: Rule = async (tree: Tree, context: SchematicContext) => {
    const eslintFilePath = '/eslint.config.json';

    if (tree.exists(eslintFilePath)) {
      const eslintFile = tree.readText(eslintFilePath);
      if (eslintFile.match(/'@o3r\/eslint-config'/)) {
        context.logger.info('You already use "@o3r/eslint-config" in your configuration');
        return;
      }
      const newContent = eslintFile.replace(
        /(module.exports = \[)/,
        `const o3rConfig = require('@o3r/eslint-config');\n\n$1\n  ...o3rConfig.default,`
      );

      tree.overwrite(eslintFilePath, newContent);
    } else {
      const { getAllFilesInTree, getTemplateFolder } = await import('@o3r/schematics');
      const eslintConfigFiles = getAllFilesInTree(tree, '/', ['**/eslint.config.js'], false).filter((file) => /eslint\.config/i.test(file));
      if (!eslintConfigFiles.length) {
        ...
        context.logger.warn(`You can manually extend "@o3r/eslint-config" in your configuration ${eslintConfigFiles.map((f) => `"${f}"`).join(', ')}`);
        return;
      }
    }
  };
  ...
  const createProjectFiles: Rule = async (tree: Tree, context: SchematicContext) => {
    ...
    const eslintFilePath = posix.join(projectRoot, '/eslint.config.js');
  ...
  const editAngularJson: Rule = async (tree: Tree, context: SchematicContext) => {
    ...
    workspaceProject.architect.lint ||= {
      ...
      options: {
        eslintConfig: `${workspaceProject.root}/eslint.config.js`,
        lintFilePatterns: [
