  ...
  return async (tree, context) => {
    ...
    const updateSpecVersion: Rule = () => {
      ...
      if (tree.exists(readmeFile)) {
        ...
        const generatorVersion = /(openapi|swagger): *([0-9]+\.[0-9]+\.[0-9]+(?:-[^ ]+)?)/.exec(specContent);

        if (specVersion && generatorVersion) {
          const readmeContent = tree.read(readmeFile)!.toString('utf8');
          tree.overwrite(readmeFile, readmeContent.replace(/Based on .+ spec .*/i, `Based on SDK spec version ${specVersion[1] || '0.0.0'} (using ${generatorVersion[1]} ${generatorVersion[2]})`));
        }
      }
      return tree;
    };
