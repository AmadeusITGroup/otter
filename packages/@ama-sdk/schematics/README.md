...
> [!NOTE]
> To align ourselves with OpenAPI 3.0, we now support arrays and objects in path and query parameters along with their serialization. Based on the values of the keywords `style` and `explode` within the specification file, the parameters are serialized accordingly in the URLs of the APIs.
> For more information, check out OpenAPI's documentation on [parameter serialization](https://swagger.io/docs/specification/v3_0/serialization/).
