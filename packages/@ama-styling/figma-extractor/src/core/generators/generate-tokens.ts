...
export const getTokensVariables = async (
  ...
  variableList: Promise<GetLocalVariables200ResponseMeta>,
  ...
  return Object.values(variables.variableCollections).reduce((acc, collection) => {
    collection.modes.forEach((mode) => {
      const tokenFile = {
        ...
        mode,
        tokens: getTokensFromLocalVariables(variables, textWeightVariableIds, { ...options, modeId: mode.modeId, collectionId: collection.id })
      };
