describe('getTokensFromLocalVariables', () => {
  test('should generate the manifest file content for color and text', () => {
    ...
    expect(result).toEqual({
      ref: { path: { $value: '{var1.ref}' } },
      str: { path: { $type: 'string', $value: 'a string' } },
      unit: { path: { $type: 'dimension', $value: '4px' } },
      'unitless/var': { path: { $type: 'dimension', $value: 4 } },
      var1: { path: { $type: 'color', $value: '#test' } }
    });
  });
});
