...
import {
  ...
  getPathFromName,
} from '../../helpers/name-to-reference.helpers';
import type {
  DesignToken,
  FigmaFileContext,
} from '../../interfaces';
...
export const getTokensFromLocalVariables = (
  ...
  const { variables } = localVariablesResponse;
  const unitLessGroups = [
    ...new Set(
      unitLessVariableIds
        .map((id) => variables[id])
        .filter((variable) => !!variable)
        .map(({ name }) => name.split('/').slice(0, -1))
        .join('/')
    )
  ];
  const filteredVariableIds = localVariablesResponse.variableCollections[options.collectionId].variableIds;
