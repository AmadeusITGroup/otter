...
To avoid this, the `@ama-mfe/ng-utils` has a special `provideDisableHistoryWrites()` that will forbid the application running in the iframe to alter the browser history. This will prevent the iframe to be able to use the `history.pushState` and `history.replaceState` methods.

```ts
import {provideDisableHistoryWrites} from "@ama-mfe/ng-utils";
