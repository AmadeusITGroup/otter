...
describe('ThemeProducerService', () => {
  ...
  let messageService: MessagePeerService<ThemeVersions>;
  ...
  beforeAll(() => {
    ...
    // eslint-disable-next-line no-undef -- window object available in the test env
  ...
  beforeEach(() => {
    ...
    messageService = TestBed.inject<MessagePeerService<ThemeVersions>>(MessagePeerService);
  });
  ...
  it('should handle errors and revert to previous theme', () => {
    const errorMessage: ErrorContent<ThemeVersions> = { reason: 'unknown_type', source: { type: 'theme', version: '1.0', name: '', theme: '' } };
