...
describe('ApplyTheme', () => {
  ...
  it('should sanitize and return the url with theme if it is a string', () => {
    const url = 'http://example.com';
    const result = pipe.transform(url);
    expect(sanitizer.sanitize).not.toHaveBeenCalled();
    expect(sanitizer.bypassSecurityTrustResourceUrl).toHaveBeenCalledWith('http://example.com/?theme=dark');
