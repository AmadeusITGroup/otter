...
export interface ActivityProducerConfig {
  /**
   * Minimum interval in milliseconds between activity messages sent to the host(shell) app.
   ...
   * Only used when trackNestedIframes is true. Defaults to 1000ms.
   */
   ...
   * Only used when trackNestedIframes is true. Defaults to 30000ms.
   */
...
export interface IframeActivityTrackerConfig {
  /** Interval in milliseconds for polling document.activeElement to detect iframe focus. Defaults to 1000ms. */
  pollIntervalMs?: number;
  /** Interval in milliseconds for sending activity signals to the host(shell) app while an iframe has focus. Defaults to 30000ms. */
