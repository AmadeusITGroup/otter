...
describe('Navigation Handler Service', () => {
  ...
  it('should call navigate when a supported message is received', () => {
    jest.spyOn(navHandlerService, 'navigate');
    const navMessage: RoutedMessage<NavigationVersions> = {
      from: 'test',
      to: [],
      payload: {
        type: 'navigation',
    ...
    expect(navHandlerService.navigate).toHaveBeenCalledWith(navMessage.payload.url, 'test');
  });
