...
function getBlock(lines: string[], blockName: string) {
  ...
  const endIndex = lines.slice(nextLineIndex).findIndex((l) => /^\w+:$/.test(l));
  if (endIndex === -1) {
    return lines.slice(nextLineIndex);
  }
  return lines.slice(
    nextLineIndex,
...
async function discoverSchematics(options: AngularSchematicOptions, logger: Logger) {
  ...
  if (useYarn) {
    ...
    try {
      ...
      await (useYarn ? run('yarn', ['--immutable'], cwd, logger) : run('npm', ['ci'], cwd, logger));
    }
  }
 ...
 * @param logger
 */
export function registerAngularSchematicsTool(server: McpServer, options: AngularSchematicOptions, logger: Logger = defaultLogger) {
