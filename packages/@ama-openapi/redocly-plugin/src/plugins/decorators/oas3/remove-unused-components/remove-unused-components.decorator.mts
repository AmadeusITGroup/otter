      ...
      leave: (node, ctx) => {
        ...
        COMPOSITION_KEYWORDS
          ...
          .forEach((item) => {
            ...
            schemas
              .filter((schema): schema is typeof schema & { $ref: string } => !!schema.$ref)
              .forEach((schema) => {
                const discriminatorRef = discriminatorRefs.find(({ location }) => location.absolutePointer.endsWith(schema.$ref));
                if (discriminatorRef) {
