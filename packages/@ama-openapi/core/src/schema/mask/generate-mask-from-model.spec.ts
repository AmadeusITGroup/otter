...
describe('generateMaskSchemaFromModel', () => {
  ...
  it('should return array schema with prefixItems as promises for array model', async () => {
    ...
    // Because implementation does not await the recursive calls,
    // prefixItems are Promises.
    expect(result.prefixItems[0]).toBeInstanceOf(Promise);
    expect(result.prefixItems[1]).toBeInstanceOf(Promise);

    const resolved = await Promise.all(result.prefixItems);
