...
A core package for managing OpenAPI dependencies and specifications in JavaScript/TypeScript projects. This package provides essential functionality to download, process, and install OpenAPI model dependencies with support for transformations and advanced dependency management.
...
The `@ama-openapi/core` package provides a dependency management system for [OpenAPI specifications](https://redocly.com/). It allows you to:

- Specify OpenAPI model dependencies in to used in a [manifest file](#manifest-configuration).
...
- Handle multi In-depth version, range version and conflict resolution thanks to [NPM version mechanism](https://docs.npmjs.com/about-npm).
...
To depend of an OpenAPI package, the dependency can be added via the command:
...
Will bring the 2 versions of the `spec2` in the final bundle generated by [Redocly](https://redocly.com/) (has there is no intersection between `~1.1.0` and `~1.2.0`).

If needed, NPM offers the feature to enforce the version over the dependencies via [overrides field](https://docs.npmjs.com/cli/v9/configuring-npm/package-json#overrides):
...
> [!IMPORTANT]
> The NPM dependency allow accessing to the models but do not allow the models to be referred in your specifications.\
> To reference the models, they need to be listed in the [manifest file](#model-extraction).
...
When the model package is available for NodeJs (cf. [NPM package dependency](#npm-package-dependency)), we need to extract the models to let them be targeted within a Specification files.

The dependency management system uses [manifest configuration files](#manifest-configuration) to define which OpenAPI to be reachable and how to process them.\
The extraction can then be process by using the the command `ama-openapi install` from [@ama-openapi/cli package](https://www.npmjs.com/package/@ama-openapi/cli).

The process will then extract the models into the directory `models_external/` which can then be referred in you specification
...
| --- | --- |
| `fileRename` | Allow renaming a model file name and allow referring the original name with the keyword `$1`. <br /> *Example: `"myPrefix_$1"` will prefix the model.* <br /> Note that the filename in used by [Redocly bundle](https://redocly.com/docs/cli/commands/bundle) to generate the final model name. |
| `titleRename` | Allow renaming a model title name and allow referring the original name with the keyword `$1`.<br /> *Example: `"myPrefix_$1"` will prefix the model title.* Note that the field `title` is ignore by Redocly. |
...
- Allow the the application of transform on top of definitions
- Support to link the `$ref` to the referred definition (thanks to [VsCode OpenApi extension](https://marketplace.visualstudio.com/items?itemName=42Crunch.vscode-openapi))
- Support of version override when exposed as dependency.

### Reference only

When there is not transform apply to the external model, the only purpose of the model file generated in `models_external` is to redirect to the correct dependency path (concretely in `node_module`).\
To simplify it, only a `$ref` will be print in the file, as following:
...
A transform model is a model generated in `models_external` directory on which one, or several, [transforms](#transforms) have been apply one.\
The model will also include additional [annotation](#model-annotations) according to the updated parts.

> [!NOTE]
> When multiple transform sets are apply to a single model, this will result to multiple model file generated in `models_external`. See [manifest examples](#manifest-schema) for more details.

### Model annotations

The following annotation, via [x-vendor](https://swagger.io/docs/specification/v3_0/openapi-extensions/) fields, are added to the models extracted to the `models_extrenal` directory:

| Annotation | Position | Description |
| --- | --- | --- |
| `x-internal-masked` | model root | Indicate that the model has been masked |
| `x-internal-touched` | model root | Indicate the model has be modified by any [transform](#transforms) |
...
| `x-internal-reference-rewritten` | beside `$ref` node | Indicate that the associated `$ref` value has been updated |
| `x-internal-reference-generated` | beside `$ref` node | Indicate that the associated `$ref` has been generated and target a local file |
...
> [!TIP]
> In case no valid file from previous the list is found, the mechanism will parse the `package.json` to find manifest definition.
...
> [!NOTE]
> The keyword `$1` allow to refer original file name, in the example the final file will be `MyPrefix_ExampleModel.v1.yaml`.

</details>

<details>

<summary>Apply single mnsk</summary>
...
> [!NOTE]
> This will result to the generation of multiple models, one for each transform object.
...
As the dependency system is based on NPM package, a shareable specification should follow the requirements for an NPM package:
...
3. The references of the different models should not break.
...
To ensure the external references to be still resolvable by the [Redocly bundle](https://redocly.com/docs/cli/commands/bundle) process when the consumer will reference one of your models, the following steps are required:

- The external references should target exclusively URL or files in `models_external/` directory.
- The external referred models's packages should be listed in `dependencies` or the `package.json`
- The external models extracted in `postinstall` live cycle of your package.
...
The package includes JSON Schema validation for manifest files. The schema is available at:

```text
@ama-openapi/core/schemas/manifest.schema.json
```

You can use this schema in your IDE or build tools to get validation and autocomplete support for manifest files.

> [!TIP]
> The project generator [@ama-openapi/create](https://www.npmjs.com/package/@ama-openapi/create) prepare the manifest configuration file (and `package.json`) to refer the `manifest.schema.json`.

### Renovate configuration

The package expose Renovate preset to help to upgrade and mutualize the dependencies relative to the Ama Openapi ecosystem at:

```text
@ama-openapi/core/renovate/default.json
```
